...
def main(argv=sys.argv, argc=len(sys.argv)):
    ...
    with open(sdt_path, "rb") as sdt:
        ...
        while (sdt.tell() < sdt_size):
            header = sdt.read(16) #read first 16 bytes
            id = get_u32_le(header, 0x00) #unsigned 32-bit little-endian
            if id == 0xF0: ##
                # end of stream found
                break
            elif id == 0x10:
